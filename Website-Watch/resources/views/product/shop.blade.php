                ...
                <div class="row">
                    <div class="col-lg-3 col-md-6 col-sm-8 order-2 order-lg-1 products-sidebar-filter">
                        ...
                        <div class="filter-wiget">
                            ...
                            <div class="bc-item">
                                <label for="g-nam">
                                    <input type="checkbox" {{ (request('gender')['nam'] ?? '') == 'on' ? 'checked' : '' }}
                                        id="g-nam" name="gender[nam]" onchange="this.form.submit()">
                                    Nam
                            ...
                            <div class="bc-item">
                                <label for="g-nu">
                                    <input type="checkbox" {{ (request('gender')['nu'] ?? '') == 'on' ? 'checked' : '' }}
                                        id="g-nu" name="gender[nu]" onchange="this.form.submit()">
                                    Nữ
                                    <span class="checkmark"></span>
                                </label>
                            </div>
                            ...
                            @else
                                ...
                                <div id="row_wishlist" class="row">
                                    @foreach ($liked as $like)
                                        @if ($like->status == 'like' && $like->customers == Auth::user()->id)
                                            <div class="row list-like-product product--{{ $like->product }}"
                                                style="margin: 10px 0" data-id="{{ $like->product }}">
                                                @php
                                                    $nameImage = $like->products->image_1;
                                                @endphp
                                                ...
                                                <div class="col-md-8">
                                                    ...
                                                    <p style="color:#fe980f">@php $price = $like->products['price']-($like->products['price'] * ($like->products['discount']/100)) @endphp
                                                        {{ number_format($price) . ' VNĐ' }}
                                                    </p>
                                                    <a class="btn btn-success"
                                                        href="/chi-tiet-san-pham/{{ $like->product }}">Chi
                                                        tiết</a>
                                ...
                                <div class="row">
                                    <div class="col-12 col-4 d-flex justify-content-end ">
                                        <button type="button" class="btn btn-success mb-1" id="buy-all-like-product">Đặt mua
                                            tất cả</button>
                                    </div>
                                </div>
                            @endguest
                                ...
                                <li>
                                        ...
                                        <div class="quick-view-controller" style="justify-content: center;">
                                            <p class="product-item-desc-button-submit">
                                                ...
                                                <input type="hidden" id="product_name{{ $product->id }}"
                                                    class="productName" value="{{ $product->name }}">
                                                ...
                                                <input type="hidden" id="product_image{{ $product->id }}"
                                                    src="{{ URL::to('/images/image_products_home/' . $product->image_1) }}"
                                                    class="productImage" value="{{ $product->image_1 }}">
                                            </p>
                                    ...
                                    <div class="p-text">
                                        ...
                                        <div class="product-price">
                                            @if ($product->discount != null)
                                                <span class="price-initial">{{ number_format($product->price) }}
                                                    VNĐ</span>
                                                <span
...
@push('scripts')
    <script>
        $(document).ready(function() {
            var min = {{ $min_price }};
            var max = {{ $max_price }};
            if ($("#start_price").val() != "" && $("#start_price").val() >= min)
                min = $("#start_price").val();
            if ($("#end_price").val() != "" && $("#end_price").val() <= max) {
                max = $("#end_price").val();
            };
            $("#slider-range").slider({
            ...
            $("#amount_start").val($("#slider-range").slider("values",
                0)).simpleMoneyFormat();
